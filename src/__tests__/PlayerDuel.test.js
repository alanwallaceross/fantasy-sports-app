...
describe("PlayerDuel", () => {
  test("complete game in 11 guesses and reset", async () => {
    ...
    await wait(() => expect(getByText("PlayerDuel")));
    ...
    await wait(() =>
      expect(getByTestId("character-15475-9524")).toBeDisabled()
    );
    await wait(() =>
      expect(getByTestId("character-15475-20848")).toBeDisabled()
    );
    await wait(() =>
      expect(getByTestId("character-15475-12363")).toBeDisabled()
    );
    await wait(() => expect(getByText("Next Guess")));
  ...
  test("renders error if can't resolve data", async () => {
    mockGetPlayers.mockImplementation(() => {
